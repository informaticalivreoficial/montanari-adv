...
<section class="content">
    ...
    <div class="card">
        <div class="card-header text-right">
            <a href="{{route('admin.artigos.create')}}" class="btn btn-default"><i class="fas fa-plus mr-2"></i> Cadastrar Novo</a>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
                    ...
                    <tbody> 
                        ...
                        <tr style="{{ ($post->status == '1' ? '' : 'background: #fffed8 !important;')  }}">                            
                            ...
                            <td>
                                ...
                                <a href="{{ route('admin.artigos.edit', [ 'artigo' => $post->id ]) }}" class="btn btn-xs btn-default"><i class="fas fa-pen"></i></a>
           ...
           $.ajaxSetup({
            ...
            $('.j_modal_btn').click(function() {
                ...
                $.ajax({
                    type: 'GET',
                    dataType: 'JSON',
                    url: '{{ route('admin.artigos.delete') }}',
                    data: {
                       'id': artigo_id
                    },
                    success:function(data) {
                        if(data.error){
                            ...
                            $('#frm').prop('action','{{ route('admin.artigos.deleteon') }}');
                        }else{
                            $('#frm').prop('action','{{ route('admin.artigos.deleteon') }}');
                        }
                    }
            ...
            $('.toggle-class').on('change', function() {
                ...
                $.ajax({
                    type: 'GET',
                    dataType: 'JSON',
                    url: '{{ route('admin.artigos.artigoSetStatus') }}',
                    data: {
                        'status': status,
                        'id': artigo_id
                    },
