...
<section class="content">
    ...
    <div class="card">
        <div class="card-header text-right">
            <a href="{{route('admin.users.create')}}" class="btn btn-default"><i class="fas fa-plus mr-2"></i> Cadastrar Novo</a>
        </div>        
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
                    ...
                    <tbody>
                        ...
                        <tr style="{{ ($user->status == '1' ? '' : 'background: #fffed8 !important;')  }}">
                            ...
                            <td>
                                ...
                                @endif 
                                <a href="{{route('admin.email.send',['id' => $user->id, 'parametro' => 'user'] )}}" class="btn btn-xs text-white bg-teal"><i class="fas fa-envelope"></i></a>                           
                                <a href="{{route('admin.users.view',['id' => $user->id])}}" class="btn btn-xs btn-info text-white"><i class="fas fa-search"></i></a>
                                <a href="{{route('admin.users.edit',['user' => $user->id])}}" class="btn btn-xs btn-default"><i class="fas fa-pen"></i></a>
           ...
           $.ajaxSetup({
            ...
            $('.j_modal_btn').click(function() {
                ...
                $.ajax({
                    type: 'GET',
                    dataType: 'JSON',
                    url: '{{ route('admin.users.deletecli') }}',
                    data: {
                       'id': user_id
                    },
                    success:function(data) {
                        if(data.error){
                            ...
                            $('#id_user').val(data.id);
                            $('#frm').prop('action','{{ route('admin.users.deleteoncli') }}');
                        }else{
                            $('#frm').prop('action','{{ route('admin.users.deleteoncli') }}');
                        }
                    }
            ...
            $('.toggle-class').on('change', function() {
                ...
                $.ajax({
                    type: 'GET',
                    dataType: 'JSON',
                    url: '{{ route('admin.users.userSetStatus') }}',
                    data: {
                        'status': status,
                        'id': user_id
                    },
