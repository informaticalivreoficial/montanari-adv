...
<section class="content">
    <div class="card">
        <div class="card-header text-right">
            <a href="{{route('admin.slides.create')}}" class="btn btn-default"><i class="fas fa-plus mr-2"></i> Cadastrar Novo</a>
        </div>        
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
                    ...
                    <tbody>
                        ...
                        <tr style="{{ ($slide->status == '1' ? '' : 'background: #fffed8 !important;')  }}">                            
                            ...
                            <td class="acoes">
                                ...
                                <a data-toggle="tooltip" data-placement="top" title="Editar Slide" href="{{route('admin.slides.edit',$slide->id)}}" class="btn btn-xs btn-default"><i class="fas fa-pen"></i></a>
           ...
           $.ajaxSetup({
                ...
                $.ajax({
                    ...
                    success:function(data) {
                        if(data.error){
                            ...
                            $('#frm').prop('action','{{ route('admin.slides.deleteon') }}');
                        }else{
                            $('#frm').prop('action','{{ route('admin.slides.deleteon') }}');
                        }
                    }
            ...
            $('.toggle-class').on('change', function() {
                ...
                $.ajax({
                    type: 'GET',
                    dataType: 'JSON',
                    url: "{{ route('admin.slides.slideSetStatus') }}",
                    data: {
                        'status': status,
                        'id': slide_id
                    },
